<script>
	...
	let className = $derived.by(() => {
		...
		if (variant === 'tech') {
			...
			return `${baseClasses} bg-yellow-100 text-yellow-800 dark:bg-yellow-900/30 dark:text-yellow-200`;
		} else {
			return `${baseClasses} bg-gray-100 text-gray-800 dark:bg-gray-800/50 dark:text-gray-200`;
		}
	});
</script>
